...
/** Connection details of the active env's running containers. Separate from
 *  `variables` because the daemon makes no assumption about which variable
 *  names the project uses — the AI maps these onto whatever names apply
 *  (e.g. MONGO_BG = `${mongoUrl}/bg`, REDIS_HOST/REDIS_PORT). */
export interface ContainersHandle {
  envId: string;
  mongoUrl?: string;
  redisUrl?: string;
  dbName: string;
...
export interface VarsListResponse {
  ...
  containers: ContainersHandle | null;
}

export interface VarsDeclareItem {
  name: string;
  value?: VarValue;
  evidence?: string;
}

export interface VarsDeclareResult {
  name: string;
...
export interface VarsDeclareResponse {
  projectName: string;
  projectRoot: string;
  results: VarsDeclareResult[];
  removed: string[];
