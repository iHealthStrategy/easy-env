// BUGGY variant of the order pipeline. Behavior toggled by BUG env var:
//   WORKER_DOWN     — worker setInterval is never started.
//   NO_IDEMP_KEY    — API skips writing the Redis idempotency key.
//   DOUBLE_INVENTORY— worker decrements inventory by 2 * qty per item.
//   NO_CONSUME      — worker marks outbox events processed but performs no
//                     side effects (no payment, no inventory, no audit, no
//                     order status change).
...
async function main() {
  ...
  app.post('/orders', async (req, res) => {
    ...
    // BUG NO_IDEMP_KEY: skip writing the idempotency key to Redis.
    if (BUG !== 'NO_IDEMP_KEY') {
  ...
  async function processOnce() {
    ...
    if (BUG === 'NO_CONSUME') {
      // Outbox is marked processed (above) but no side effects happen.
      return;
    }
    ...
    for (const it of order.items) {
      // BUG DOUBLE_INVENTORY: decrement twice the quantity.
      const dec = BUG === 'DOUBLE_INVENTORY' ? it.qty * 2 : it.qty;
      await db
        .collection('inventory')
        .updateOne(
          { _id: it.sku },
          { $inc: { stock: -dec }, $set: { updatedAt: new Date() } }
        );
    }
  ...
  // BUG WORKER_DOWN: never start the worker interval.
  if (BUG !== 'WORKER_DOWN') {
    setInterval(() => {
...
main().then((app) => {
  app.listen(PORT, () => console.log(`[mini-orders buggy BUG=${BUG || 'none'}] listening on :${PORT}`));
});
