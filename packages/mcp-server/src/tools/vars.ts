...
export const varsListToolDescription = {
  name: 'vars.list',
  description:
    "Return every environment variable easy-env knows about for the given project, plus a separate `containers` handle when an active env is up. variables: { [name]: { value, source: 'user' | 'unset' } } — values come from ~/.easy-env/projects/{projectName}/vars.json. Template placeholders inside stored values are interpolated at read time against the active env: ${mongo.url} (base only, NO query, e.g. mongodb://host:port), ${mongo.params} (query suffix like \"?replicaSet=rs0&directConnection=true\" or empty), ${mongo.host}, ${mongo.port}, ${mongo.dbName}, ${redis.url}, ${redis.host}, ${redis.port}, ${rabbit.url}, ${rabbit.host}, ${rabbit.port}, ${clickhouse.url} (http://host:port), ${clickhouse.host}, ${clickhouse.port}, ${clickhouse.dbName}, ${clickhouse.cluster} (only set when env.init declared clickhouse.cluster). Idiomatic mongo URL with db: `${mongo.url}/<dbname>${mongo.params}`. Idiomatic ClickHouse URL with db: `${clickhouse.url}/?database=${clickhouse.dbName}`. Unknown placeholders are left as-is. containers: { envId, mongoUrl?, redisUrl?, rabbitUrl?, rabbitManagementUrl?, clickhouseUrl?, clickhouseDbName?, clickhouseCluster?, dbName?, mongoHostPort?, redisHostPort?, rabbitHostPort?, clickhouseHostPort? } | null — mongoUrl in containers is the FULL working URL (with query), in case you'd rather plumb it yourself with vars.set.",
