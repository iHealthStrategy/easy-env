...
export const TOOL_DESCRIPTIONS_ZH: Record<string, string> = {
  ...
  'env.up':
    '通过 Testcontainers 为本项目启动一个全新的隔离环境。镜像版本取自 easy-env.json(backends.mongo.image / backends.redis.image),分配动态端口。返回 envId 及解析后的连接 URL,后续工具可通过 envId 引用。默认将该 env 设为 active,后续工具调用未指定 envId 时使用它。',
  'env.list':
    '列出本机上 easy-env 当前管理的所有环境,以及 active 指针指向哪个(其他工具未传 envId 时的默认目标)。可用于发现共享主机的其他 agent/session。',
  'env.status':
    '查看指定环境的详细状态:生命周期阶段、解析后的 URL、Mongo/Redis 的实时健康探针结果。',
  'env.reset':
